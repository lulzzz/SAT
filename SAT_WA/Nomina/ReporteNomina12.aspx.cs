...
{
    public partial class ReporteNomina12 : System.Web.UI.Page
    {
        #region Eventos
        ...
        protected void lnkImprimirNomina_Click(object sender, EventArgs e)
        {
            ...
            if (gvNominaEncabezado.DataKeys.Count != 0)
            {
                ...
                //Obteniendo Ruta            
                string urlReporte = TSDK.Base.Cadena.RutaRelativaAAbsoluta("~/Nomina/ReporteNomina12.aspx", "~/RDLC/Reporte.aspx");
                //Instanciando nueva ventana de navegador para apertura de registro
                TSDK.ASP.ScriptServer.AbreNuevaVentana(string.Format("{0}?idTipoReporte={1}&idRegistro={2}", urlReporte, "ComprobanteNominaN12", Convert.ToInt32(gvNominaEncabezado.SelectedDataKey["Id"])), "ComprobanteNominaN12", "location=NO,toolbar=NO,scrollbars=YES,menubar=NO,status=YES,width=800,height=500", Page);
        ...
        private void inicializaPagina()
        {
            ...
            using (
                SAT_CL.Global.CompaniaEmisorReceptor cer = new SAT_CL.Global.CompaniaEmisorReceptor(
        ...
        private void buscaNominaEncabezado()
        {
            ...
            //Obteniendo Reporte
            using (DataTable dtNominaEncabezado = SAT_CL.Nomina.Reporte.ObtieneNominaEncabezadoV_1_2(Convert.ToInt32(Cadena.RegresaCadenaSeparada(txtCompania.Text, "ID:", 1, "0")),Convert.ToInt32(txtNoNomina.Text == "" ? "0" : txtNoNomina.Text), fec_ini_pago, fec_fin_pago, fec_ini_nomina, fec_fin_nomina))
            {
                //Validando que exista el Reporte
                if (Validacion.ValidaOrigenDatos(dtNominaEncabezado))
                {
                    //Cargando GridView
                    Controles.CargaGridView(gvNominaEncabezado, dtNominaEncabezado, "Id", lblOrdenado.Text, true, 1);

                    //Añadiendo Resultado a Sesión
                    Session["DS"] = OrigenDatos.AñadeTablaDataSet((DataSet)Session["DS"], dtNominaEncabezado, "Table");
                }
                else
                {
