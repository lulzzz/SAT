...
namespace SAT.ControlPatio
{
    public partial class ControlAcceso : System.Web.UI.Page
    {
                ...
                case "Agregar":
                    {   //Validando Tabla de Session
                        ...
                        {   //Obteniendo Valor Maximo
                            ...
                            //Agregando Fila
                            ((DataSet)Session["DS"]).Tables["Table"].Rows.Add(max + 1, 0, Convert.ToInt32(TSDK.Base.Cadena.RegresaCadenaSeparada(txtTransportista.Text, "ID:", 1)),
                                         id_tipo_detalle, tipo_detalle, txtDescripcion.Text, txtIdentificador.Text, ddlEstado.SelectedValue, ddlEstado.SelectedItem.Text);
                        }
                        else
                        {   //Declarando Tabla
                            ...
                            {   //Añadiendo Columnas
                                ...
                                //Agregando Fila
                                dtUnidades.Rows.Add(1, 0, Convert.ToInt32(TSDK.Base.Cadena.RegresaCadenaSeparada(txtTransportista.Text, "ID:", 1)),
                                    id_tipo_detalle, tipo_detalle, txtDescripcion.Text, txtIdentificador.Text, ddlEstado.SelectedValue, ddlEstado.SelectedItem.Text);
        ...
        {   //Invocando Método de Limpieza de Controles
            limpiaControles();
            //Validando si existe una Seleccion
            txtTag.Focus();
            ...
            {   //Seleccionando Fila
                ...
                {   //Validando 
                    ...
                    {   //Validando que existe el Transportista
                        if (transportista.id_transportista_patio > 0)
                            //Asignando Valor
                            txtTransportista.Text = transportista.nombre + " ID:" + transportista.id_transportista_patio.ToString();
            ...
            {   //Operador
                case "3":
                    {   //Configurando Controles
                        ...
                        lblIdentificador.Text = "Identificación";
                        ...
                        if (TSDK.Datos.Validacion.ValidaOrigenDatos((DataSet)Session["DS"], "Table"))
                        {
                            ...
                            using (TransactionScope trans = TSDK.Datos.Transaccion.InicializaBloqueTransaccional(System.Transactions.IsolationLevel.ReadCommitted))
                            {
                                ...
                                if (result.OperacionExitosa)
                                {
                                    ...
                                    {
                                        //Insertando Detalle
                                        result = DetalleAccesoPatio.InsertaDetalleAccesoPatio(idAccesoEntrada, 0, Convert.ToInt32(dr["IdTransportista"]),
                                            0, 0, dr["Estado"].ToString() == "Cargado" ? true : false, fecha, Convert.ToByte(dr["IdTipo"]), dr["Descripcion"].ToString(),
                                            dr["Identificador"].ToString(), 0, ((SAT_CL.Seguridad.Usuario)Session["usuario"]).id_usuario);
        ...
        {
            //Instanciando Unidad
            using (SAT_CL.Global.Unidad uni = new SAT_CL.Global.Unidad(((SAT_CL.Seguridad.UsuarioSesion)Session["usuario_sesion"]).id_compania_emisor_receptor, txtTag.Text))
            {
                txtIdentificador.Text = uni.placas;
                txtDescripcion.Text = uni.numero_unidad;
                ddlEstado.Focus();
            }
        }

        protected void txtSTag_TextChanged(object sender, EventArgs e)
        {
            //Instanciando Unidad
            using (SAT_CL.Global.Unidad uni = new SAT_CL.Global.Unidad(((SAT_CL.Seguridad.UsuarioSesion)Session["usuario_sesion"]).id_compania_emisor_receptor, txtSTag.Text))
            {
                txtBPlacas.Text = uni.placas;
                txtBDescripcion.Text = uni.numero_unidad;
                //ddlEstado.Focus();
            }
        }
    }
