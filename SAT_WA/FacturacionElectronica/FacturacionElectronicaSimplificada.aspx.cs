...
{
    public partial class FacturacionElectronicaSimplificada : System.Web.UI.Page
    {
        #region Eventos
        ...
        #region Eventos GridView "Servicio x Facturar"
        ...
        protected void lkbAddenda_Click(object sender, EventArgs e)
        {
            ...
            if (gvServicioxFacturar.DataKeys.Count > 0)
            {
                ...
                using (FacturadoFacturacion Fac = new FacturadoFacturacion(FacturadoFacturacion.ObtieneRelacionFactura(Convert.ToInt32(gvServicioxFacturar.SelectedDataKey["NoFactura"]))))
                {
                    ...
                    {
                        //CargaCatalogo
                       SAT_CL.CapaNegocio.m_capaNegocio.CargaCatalogo
        ...
        protected void lkbAccesorios_Click(object sender, EventArgs e)
        {
            ...
            if (gvServicioxFacturar.DataKeys.Count > 0)
            {
                ...
                switch (lnk.CommandName)
                {
                    case "PDF":
                        //Obteniendo Ruta
                        string urlReporte = TSDK.Base.Cadena.RutaRelativaAAbsoluta("~/FacturacionElectronica/FcaturacionElectronicaSimplificada.aspx", "~/RDLC/Reporte.aspx");
                        ...
                        using (FacturadoFacturacion objFacturaFacturacion = new FacturadoFacturacion((FacturadoFacturacion.ObtieneRelacionFactura(Convert.ToInt32(gvServicioxFacturar.SelectedDataKey["NoFactura"])))))
                        {
                            ...
                            if (objFacturaFacturacion.id_factura_electronica > 0)
                            {
                                ...
                                using (SAT_CL.FacturacionElectronica.Comprobante objComprobante = new SAT_CL.FacturacionElectronica.Comprobante(objFacturaFacturacion.id_factura_electronica))
                                {
                                    ...
                                    if (objComprobante.id_comprobante > 0)
                                    {
                                        ...
                                        TSDK.ASP.ScriptServer.AbreNuevaVentana(string.Format("{0}?idTipoReporte={1}&idRegistro={2}", urlReporte, "Comprobante", objFacturaFacturacion.id_factura_electronica), "Comprobante", "location=NO,toolbar=NO,scrollbars=YES,menubar=NO,status=YES,width=800,height=500", Page);
                                    }
                                }
                            }
                    ...
                    case "VerComprobante":
                        ...
                        using (Facturado facturado = new Facturado(Convert.ToInt32(gvServicioxFacturar.SelectedDataKey["NoFactura"])))
                        {
                            ...
                            using (FacturadoFacturacion objFacturaFacturacion = new FacturadoFacturacion((FacturadoFacturacion.ObtieneRelacionFactura(Convert.ToInt32(gvServicioxFacturar.SelectedDataKey["NoFactura"])))))
                            {
                                ...
                                {
                                    //Obteniendo Ruta
                                    string url = TSDK.Base.Cadena.RutaRelativaAAbsoluta("~/FacturacionElectronica/FacturacionElectronicaSimplificada.aspx", "~/FacturacionElectronica/Comprobante.aspx");

                                    //Estatus de Lectura
                                    Session["estatus"] = Pagina.Estatus.Lectura;

                                    //Instanciando nueva ventana de navegador para apertura de registro
                                    Response.Redirect(string.Format("{0}?idRegistro={1}", url, objFacturaFacturacion.id_factura_electronica));
                                }
                                else
                    ...
                    case "Registrar CFDI":
                        ...
                        using (Facturado fac = new Facturado(Convert.ToInt32(gvServicioxFacturar.SelectedDataKey["NoFactura"])))
                        {
                            ...
                            if (fac.id_servicio != 0)
                            {
                                ...
                                using (SAT_CL.Documentacion.Servicio objServicio = new SAT_CL.Documentacion.Servicio(Convert.ToInt32(gvServicioxFacturar.SelectedDataKey["IdServicio"])))
                                {
                                    //Cargando Cuentas Pago 
                                    SAT_CL.CapaNegocio.m_capaNegocio.CargaCatalogo(ddlCuenta, 42, "NO IDENTIFICADO", 25, "", objServicio.id_cliente_receptor, "");
                                }
                            }
                        }
        ...
        protected void btnAceptarTimbrarFacturacionElectronica_Click(object sender, EventArgs e)
        {
            ...
            {
                //Registramos Factura
                resultado = objFacturado.TimbraFacturadoComprobante_V3_2(txtSerieFS.Text, chkOmitirAddenda.Checked, HostingEnvironment.MapPath("~/XSLT/FacturacionElectronica/cadenaoriginal_3_2.xslt"),
        ...
        private RetornoOperacion RegistraFacturacionElectronica()
        {
            ...
            {
                //Registramos Factura
                resultado = fac.ImportaFacturadoComprobante_V3_2(Convert.ToByte(ddlFormaPago.SelectedValue), Convert.ToInt32(ddlCuenta.SelectedValue), Convert.ToByte(ddlMetodoPago.SelectedValue), Convert.ToByte(ddlTipoComrobante.SelectedValue), Convert.ToInt32(ddlSucursal.SelectedValue),
                            ((SAT_CL.Seguridad.UsuarioSesion)Session["usuario_sesion"]).id_compania_emisor_receptor, ((SAT_CL.Seguridad.Usuario)Session["usuario"]).id_usuario, obtieneReferencias().TrimEnd(','));
                                        ...
                                        using (DataTable dtAplicaciones = SAT_CL.CXC.FichaIngresoAplicacion.ObtieneFichasFacturas(0, Convert.ToInt32(gvServicioxFacturar.SelectedDataKey["NoFactura"])))
                                        {
                                            ...
                                            if (TSDK.Datos.Validacion.ValidaOrigenDatos(dtAplicaciones))
                                            {
                                                ...
                                                if (resultado.OperacionExitosa)
                                                    ...
                                                    resultado = facturado.ActualizaEstatusFactura(Facturado.EstatusFactura.Registrada, ((SAT_CL.Seguridad.Usuario)Session["usuario"]).id_usuario);

                                            }
                                            else
                                                //Instanciando Factura
                                                resultado = new RetornoOperacion(Convert.ToInt32(gvServicioxFacturar.SelectedDataKey["NoFactura"]));
        ...
        private void cargaCatalogos()
        {
            ...
            SAT_CL.CapaNegocio.m_capaNegocio.CargaCatalogoGeneral(ddlTipoComrobante, "", 1100);
            //Sucursales
        ...
        private void servicioxFacturar()
        {
            ...
            //Obteniendo Reporte de Saldos Detalle
            using (DataTable dtServiciosxFacturar = SAT_CL.CXC.Reporte.ObtieneReporteServiciosxFacturar(Convert.ToInt32(TSDK.Base.Cadena.RegresaCadenaSeparada(txtClienteFS.Text, "ID:", 1)), fec_ini, fec_fin,
                                                        ...
                                                        txtReferencia.Text, txtServicioFS.Text, txtPorte.Text, chkSinRegistrar.Checked, chkRegistradas.Checked, chkTimbradas.Checked))
            {
                //Validando que existan Registros
