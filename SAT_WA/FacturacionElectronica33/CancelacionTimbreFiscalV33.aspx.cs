...
{
    public partial class CancelacionTimbreFiscalV33 : System.Web.UI.Page
    {
        #region Eventos
        /// <summary>
                    ...
                    switch (boton.CommandName)
                    {
                        case "Bitacora":
                            {   //Visualizando bitácora de registro
                                inicializaBitacoraRegistro(gvComprobante.SelectedDataKey.Value.ToString(), "209", "Bitacora");
                                break;
                            }
                        case "Referencias":
                            {   //Visualizando referencia de registro
                                inicializaReferencias(gvComprobante.SelectedDataKey.Value.ToString(), "209", "Referencias");
                                break;
                            }
                        case "Comprobante":
        ...
        {   
            ...
            using (DataTable mit = SAT_CL.FacturacionElectronica33.Reporte.CargaCancelacionTimbreFiscalV33(((SAT_CL.Seguridad.UsuarioSesion)Session["usuario_sesion"]).id_compania_emisor_receptor, Convert.ToByte(ddlTipo.SelectedValue),
        ...
        {   //Validando Si el GridView contiene Registros
            ...
            {   //Declarando variables para armado de URL
                string urlDestino = TSDK.Base.Cadena.RutaRelativaAAbsoluta("~/FacturacionElectronica33/CancelacionTimbreFiscalV33.aspx", "~/Accesorios/BitacoraRegistro.aspx?idT=" + id_tabla + "&idR=" + id_registro + "&tB=" + titulo);
        ...
        {
            //Construyendo URL 
            string url = TSDK.Base.Cadena.RutaRelativaAAbsoluta("~/FacturacionElectronica33/CancelacionTimbreFiscalV33.aspx", "~/Accesorios/ReferenciaRegistro.aspx?idT=" + id_tabla + "&idR=" + id_registro + "&idC=" + ((SAT_CL.Seguridad.UsuarioSesion)Session["usuario_sesion"]).id_compania_emisor_receptor + "&tB=" + nombre_tabla);
